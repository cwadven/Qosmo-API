...
class NodeCompletionService:
    ...
    def process_nodes_completion(self, nodes: List[Node]) -> None:
        ...
        while nodes_to_process:
            ...
            unlocked_nodes = self._find_unlocked_nodes(map_data)
            ...
            for unlocked_node_id, rule in unlocked_nodes:
                ...
                map_data['completed_node_ids'].add(unlocked_node_id)
                map_data['completed_node_rule_ids'].add(rule.id)
                nodes_to_process.append(unlocked_node_id)
    ...
    def _find_unlocked_nodes(self, map_data: Dict) -> List[tuple[int, NodeCompleteRule]]:
        """
        해금 조건이 충족된 Node들을 찾습니다.
        하나의 Node는 여러 규칙으로 해금될 수 있으며,
